<template>
  ...
  export default {
    setup(props) {
      // リアクティブデータ:自分で取得処理をせず、ストリームから送られてきたデータに反応して処理
      const data = reactive({
        title:'',
        content:'',
        ...
        // 検索実行中を表すフラグ(true:検索中、false:未検索)
        find_flg: false,

        // 項目を選択したかの変数(true:選択中/false:未選択)
        sel_flg: false,

        // 項目が選択されているときに、選択された項目のオブジェクトを保管する
        sel_item: null,

        // Vuexのストア(store.js)を代入しておくための物。Vuexに用意されたuseStore()関数を使って行う。
        store: useStore(),
      })
